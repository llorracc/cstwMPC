function [ThetaVals, ThetaProb] = MakeShocks(NumOfThetaShockPoints,Mu,Sigma)
%==========================================================================%
...
for i = 0:NumOfThetaShockPoints;
    ...
    X(i+1) = logninv(P,Mu-1/2*(Sigma)^2,Sigma);
end;
%X(NumOfThetaShockPoints+1)=100000000000;
X(NumOfThetaShockPoints+1)=1 + Sigma*100;

for i=1:NumOfThetaShockPoints;
    ProbOfMeanPoints(i) = logncdf(X(i+1),Mu-1/2*(Sigma)^2,Sigma)-logncdf(X(i),Mu-1/2*(Sigma)^2,Sigma);
end
F = @(x) x.*lognpdf(x,Mu-1/2*(Sigma)^2,Sigma);
for i=1:NumOfThetaShockPoints
    MeanPoints(i)=integral(F,X(i),X(i+1),'RelTol',10^-12)/ProbOfMeanPoints(i);
end
