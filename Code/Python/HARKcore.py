NullFunc = lambda : None # A trivial function that takes no arguments and returns None
from HARKutilities import getArgNames
...
def solveAgent(agent):
    ...
    while go:
        ...
        if infinite_horizon:
            go = (not agent.isSameThing(solution_last,solution_now)) and (completed_cycles < max_cycles)
        else:
            cycles_left += -1
            go = cycles_left > 0
