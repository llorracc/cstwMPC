(* FindBetaDistSevenFin.m *)

(* Find \[Beta]middle which matches distribution of FINANCIAL assets levels, using Dist model with time pref factors approximated by seven points *)
...
Initial\[Beta]middle           = 0.9750;           (* Initial guess of \[Beta]middle *) 
Initialdiff                    = 0.0025;           (* Initial value of diff (difference between approximated points of time pref factor) *)
diff                           = Initialdiff; 
...
Print["Obtain (common) initial distribution of Pt and kRatt for later simulations "];
 ...
 <<FindIntBetaDistSeven_ReiterFin.m; (* (The Reiter's method is) used to get good guess. *)
...
ConstructShockLists;      (* Construct shock lists *)
 ...
 StatsMat = Table[
  ...
  <<FindIntBetaDistSeven_ReiterFin.m; (* (The Reiter's method is) used to get good initial guess. *)
...
Export["kLevListDistSevenNoAggShockFin.txt", kLevListDistSevenNoAggShock, "Table"];
Export[ParentDirectory[] <> "/Results/kLevListDistSevenNoAggShockFin.txt", kLevListDistSevenNoAggShock, "Table"];
Export[ParentDirectory[] <> "/Results/MPCDistSevenNoAggShockFin.txt", AggMPCMeanAnnual, "Table"];
Export[ParentDirectory[] <> "/Results/KYDistSevenNoAggShockFin.txt", (KLevMean/lbar/L)^(1 - CapShare), "Table"];

Export["diffFin.txt", diff, "Table"];
Export["BetamiddleFin.txt", \[Beta]middleSolution, "Table"];
Export[ParentDirectory[] <> "/WithAggShocks/diffFin.txt", diff, "Table"];
Export[ParentDirectory[] <> "/WithAggShocks/BetamiddleFin.txt", \[Beta]middleSolution, "Table"];
Export[ParentDirectory[] <> "/WithAggPermShocks/diffFin.txt", diff, "Table"];
Export[ParentDirectory[] <> "/WithAggPermShocks/BetamiddleFin.txt", \[Beta]middleSolution, "Table"];

Export[ParentDirectory[] <> "/Results/BetamiddleFin.tex", Round[10000 \[Beta]middleSolution]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/NablaFin.tex",      Round[10000 3.5 diff]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/BetaLowFin.tex", Round[10000 (\[Beta]middleSolution - 3.5 diff)]/10000 // N, "Table"];
Export[ParentDirectory[] <> "/Results/BetaHighFin.tex",Round[10000 (\[Beta]middleSolution + 3.5 diff)]/10000 // N, "Table"];
