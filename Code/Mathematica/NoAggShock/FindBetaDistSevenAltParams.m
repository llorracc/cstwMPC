...
(* Find \[Beta]middle which matches distribution of wealth levels, using \[Beta]-Dist model with time pref factors approximated by seven points *)
...
Initial\[Beta]middle           = Import["Betamiddle.txt","List"][[1]]; (* Initial guess of \[Beta]middle *) 
Initialdiff                    = Import["diff.txt","List"][[1]];       (* Initial value of diff (difference between approximated points of time pref factor) *)
diff                           = Initialdiff; 
...
Export[ParentDirectory[] <> "/WithAggShocks/diffAltParams.txt", diff, "Table"];
Export[ParentDirectory[] <> "/WithAggShocks/BetamiddleAltParams.txt", \[Beta]middleSolution, "Table"];
Export[ParentDirectory[] <> "/WithAggPermShocks/diffAltParams.txt", diff, "Table"];
Export[ParentDirectory[] <> "/WithAggPermShocks/BetamiddleAltParams.txt", \[Beta]middleSolution, "Table"];
Export[ParentDirectory[] <> "/Results/BetamiddleAltParams.tex", Round[10000 \[Beta]middleSolution]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/NablaAltParams.tex",      Round[10000 3.5 diff]/10000//N, "Table"];
