(* FindBetaDistSevenLiqFin.m *)

(* Find \[Beta]middle which matches distribution of Liquid FINANCIAL assets levels, using Dist model with time pref factors approximated by seven points *)
...
Print["============================================================="];
Print["Match liquid FINANCIAL assets "];
...
MatchLiqFinAssets  = Yes;    (* Mathcing liquid financial assets, not net worth *)


(* Set parameter values and set up routines *)
Initial\[Beta]middle           = 0.910;             (* Initial guess of \[Beta]middle *) 
Initialdiff                    = 0.0095;            (* Initial value of diff (difference between approximated points of time pref factor) *)
diff                           = Initialdiff; 
...
Print["Obtain (common) initial distribution of Pt and kRatt for later simulations "];
 ...
 <<FindIntBetaDistSeven_ReiterLiqFin.m; (* (The Reiter's method is) used to get good guess. *)

(* 
(* Obtaining inital dist in this way does not work well in LiqFin case because KLevMean is far off from KSS. Doing this in net worth and Fin cases does not create a problem KLevMean is close enough. *)
...
ConstructShockLists;      (* Construct shock lists *)
 ...
 StatsMat = Table[
  ...
  <<FindIntBetaDistSeven_ReiterLiqFin.m; (* (The Reiter's method is) used to get good initial guess. *)
...
Export["kLevListDistSevenNoAggShockLiqFin.txt", kLevListDistSevenNoAggShock, "Table"];
Export[ParentDirectory[] <> "/Results/kLevListDistSevenNoAggShockLiqFin.txt", kLevListDistSevenNoAggShock, "Table"];
Export[ParentDirectory[] <> "/Results/MPCDistSevenNoAggShockLiqFin.txt", AggMPCMeanAnnual, "Table"];
Export[ParentDirectory[] <> "/Results/KYDistSevenNoAggShockLiqFin.txt", (KLevMean/lbar/L)^(1 - CapShare), "Table"];

Export["diffLiqFin.txt", diff, "Table"];
Export["BetamiddleLiqFin.txt", \[Beta]middleSolution, "Table"];
Export[ParentDirectory[] <> "/WithAggShocks/diffLiqFin.txt", diff, "Table"];
Export[ParentDirectory[] <> "/WithAggShocks/BetamiddleLiqFin.txt", \[Beta]middleSolution, "Table"];
Export[ParentDirectory[] <> "/WithAggPermShocks/diffLiqFin.txt", diff, "Table"];
Export[ParentDirectory[] <> "/WithAggPermShocks/BetamiddleLiqFin.txt", \[Beta]middleSolution, "Table"];

Export[ParentDirectory[] <> "/Results/BetamiddleLiqFin.tex", Round[10000 \[Beta]middleSolution]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/NablaLiqFin.tex",      Round[10000 3.5 diff]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/BetaLowLiqFin.tex", Round[10000 (\[Beta]middleSolution - 3.5 diff)]/10000 // N, "Table"];
Export[ParentDirectory[] <> "/Results/BetaHighLiqFin.tex",Round[10000 (\[Beta]middleSolution + 3.5 diff)]/10000 // N, "Table"];
...
Clear[MatchLiqFinAssets]; (* Clear MatchLiqFinAssets var *)
