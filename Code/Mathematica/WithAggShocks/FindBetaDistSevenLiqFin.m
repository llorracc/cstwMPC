...
(* Find \[Beta]middle which matches distribution of liquid FINANCIAL assets levels, using Dist model with time pref factors approximated by seven points. Aggregate shocks are on. *)
...
\[Beta]middle = Import["BetamiddleLiqFin.txt", "List"][[1]];
Export["BetamiddleOriginalLiqFin.txt", \[Beta]middle, "Table"]; 

(*  Initial params  *)
initialdiff = 0.0120;                 (* Initial difference beteen approximated points of \[Beta] *)
initial\[Beta]middle = \[Beta]middle; (* Use \[Beta]middle estimated in the model without agg shock *)
Export["BetamiddleLiqFin.txt", initial\[Beta]middle, "Table"]; 
...
StatsMat = Table[
   ...
   Find\[Beta]middleSolutionLiqFin; 
   Export["BetamiddleLiqFin.txt", Import["BetamiddleOriginalLiqFin.txt", "List"][[1]], "Table"]; (* Recover initial Betamiddle *)
...
Export["diffWithAggShocksLiqFin.txt", diffSolution, "Table"];
Export["BetamiddleWithAggShocksLiqFin.txt", \[Beta]middleSolution, "Table"];

Export[ParentDirectory[] <> "/Results/BetamiddleWithAggShocksLiqFin.tex",  Round[10000 \[Beta]middleSolution]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/NablaWithAggShocksLiqFin.tex",       Round[10000 3.5 diffSolution]/10000//N, "Table"];
...
Export["BetamiddleLiqFin.txt", Import["BetamiddleOriginalLiqFin.txt", "List"][[1]], "Table"];
