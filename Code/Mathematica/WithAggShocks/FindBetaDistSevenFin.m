...
(* Find \[Beta]middle which matches distribution of FINANCIAL assets levels, using Dist model with time pref factors approximated by seven points. Aggregate shocks are on. *)
...
\[Beta]middle = Import["BetamiddleFin.txt", "List"][[1]];
Export["BetamiddleOriginalFin.txt", \[Beta]middle, "Table"]; 

(*  Initial params  *)
initialdiff = 0.0040;                 (* Initial difference beteen approximated points of \[Beta] *)
(*
initialdiff = 0.0035;                 (* Initial difference beteen approximated points of \[Beta] *)
*)
initial\[Beta]middle = \[Beta]middle; (* Use \[Beta]middle estimated in the model without agg shock *)
Export["BetamiddleFin.txt", initial\[Beta]middle, "Table"]; 
...
StatsMat = Table[
   ...
   Find\[Beta]middleSolutionFin; 
   Export["BetamiddleFin.txt", Import["BetamiddleOriginalFin.txt", "List"][[1]], "Table"]; (* Recover initial Betamiddle *)
...
Export["diffWithAggShocksFin.txt", diffSolution, "Table"];
Export["BetamiddleWithAggShocksFin.txt", \[Beta]middleSolution, "Table"];

Export[ParentDirectory[] <> "/Results/BetamiddleWithAggShocksFin.tex",  Round[10000 \[Beta]middleSolution]/10000//N, "Table"];
Export[ParentDirectory[] <> "/Results/NablaWithAggShocksFin.tex",       Round[10000 3.5 diffSolution]/10000//N, "Table"];
...
Export["BetamiddleFin.txt", Import["BetamiddleOriginalFin.txt", "List"][[1]], "Table"];
