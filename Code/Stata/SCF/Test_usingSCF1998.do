cd "C:\Economics\JEDC project\BSinKS-private\cstCode\Latest\Code\Stata\SCF"

* This file i) calculates the fraction of households whose wealth level is between 0.5*mean and 1.5*mean, and ii) the distribution of wealth/permanent income ratio, using SCF1998
...
gen FINExRet = LIQ+CDS+NMMF+STOCKS+BOND        +SAVBND+CASHLI+OTHMA+OTHFIN /* Total fin asset ex retirement assets*/
...
gen CINCOME    = x5729        /* current income */
replace PINCOME = x7362 if x7650!=3
drop if PINCOME <=0  
replace PINCOME = PINCOME*0.87 /* Adjust with CPI (adjusted to 1992$ price) */    
sum PINCOME [weight= WGT]
gen NPINCOME = PINCOME /49419.12      /* P/M[P] */
sum NPINCOME  [weight= WGT]

log using "results-Test_usingSCF1998", replace text

sum LiqFIN  [weight= WGT]
sum FINExRet  [weight= WGT]
sum FIN  [weight= WGT]

sum RETQLIQ  [weight= WGT]
sum CASHLI  [weight= WGT]
sum OTHMA  [weight= WGT]
sum OTHFIN  [weight= WGT]
...
* SCF1998
gen MFINYRATIO = (FIN+CINCOME*0.87/4)/(PINCOME/4) /* note that current inc in the prev yr was reported */
...
* FINExRetYRATIO (FINExRetanical assets/perm inc ratio) by percentile 
log using "results-WYDistFINExRet", append text
* SCF1998
replace FINExRet    = FINExRet*0.86
gen FINExRetYRATIO  = FINExRet/(PINCOME/4)
 
sum FINExRetYRATIO [weight= WGT]  if FINExRetYRATIO <10000, d
pctile pctFINExRet = FINExRetYRATIO [weight=WGT] if FINExRetYRATIO <10000, nq(40) genp(percentFINExRet)
list percentFINExRet  pctFINExRet in 1/40

* MFINExRetYRATIO (current income + FINExRetancial assets/perm inc ratio) by percentile 
* SCF1998
gen MFINExRetYRATIO = (FINExRet+CINCOME*0.87/4)/(PINCOME/4) /* note that current inc in the prev yr was reported */
sum MFINExRetYRATIO [weight= WGT]  if MFINExRetYRATIO <10000, d
pctile pctMFINExRet = MFINExRetYRATIO [weight=WGT] if MFINExRetYRATIO <10000, nq(20) genp(percentMFINExRet)
list percentMFINExRet  pctMFINExRet in 1/20
...
* SCF1998
gen MLiqFINYRATIO = (LiqFIN+CINCOME*0.87/4)/(PINCOME/4) /* note that current inc in the prev yr was reported */
...
***************
** Cumulative distribution of financial assets ex retirement assets 
* normalize weight
gsort -FINExRet  
egen float TempWGT = pc( WGT)
drop WGT
...
* calculate cumulative distribution from the top 
gen TempFINExRet  = FINExRet *WGT
egen float DistFINExRet  = pc( TempFINExRet )
gen cumFINExRet      = DistFINExRet[1]
replace cumFINExRet  = DistFINExRet[_n] + cumFINExRet[_n-1] if _n>1

* show financial assets held by top x% (note that the order is reversed from RATIOS (above))
log using "results-FINExRetCumDist", append text
* SCF1998
sum cumFINExRet [weight= WGT] , d
pctile pctcumFINExRet  = cumFINExRet [weight=WGT], nq(20) genp(percentFExRet )
list percentFExRet  pctcumFINExRet  in 1/20

/*
* plot 
gen     percentileFINExRet  = WGT[1]
replace percentileFINExRet  = WGT[_n] + percentileFINExRet[_n-1] if _n>1
replace percentileFINExRet  = (100-percentileFINExRet *100)
gen CumFINExRet Dist = 100-cumFINExRet 
line CumFINExRet Dist percentileFINExRet 
*/

* AGGREGATE financial assets ex retirement assets/ perm inc ratio (note divided by quarterly perm inc)  
gen Temp = FINExRet  * WGT
egen float MeanFINExRet  = total(Temp)
drop Temp
gen AggFINExRetYRATIO = MeanFINExRet /(MeanPINCOME/4)
mean AggFINExRetYRATIO /* show aggregate FINExRet YRATIO */
