* This file i) calculates the fraction of households whose wealth level is between 0.5*mean and 1.5*mean, and ii) the distribution of wealth/permanent income ratio, using SCF1995
...
drop if PINCOME <0  
replace PINCOME = PINCOME*0.95 /* Adjust with CPI (adjusted to 1992$ price) */   
sum PINCOME [weight= WGT]
gen NPINCOME = PINCOME /47594.27      /* P/M[P] */
...
* LiqFINPlsRetYRATIO (liq finanical assets plus ret asset/perm inc ratio) by percentile 
log using "results-WYDistLiqFINPlsRet", append text
* SCF1995
replace LiqFINPlsRet    = LiqFINPlsRet*0.92
